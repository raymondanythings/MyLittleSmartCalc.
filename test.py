...
btns = ["C741.", f"{weather}8520", f"{covid}963?", "/-+X="]
    ...
    for col, titles in enumerate(btns):
        for row, txt in enumerate(titles):
            b1 = Button(bottom_frame, text=txt,
                        font=("Courier"), width=2, command=lambda cmd=txt: btn_click(cmd, bottom_frame, bottom_frame2))
    ...
    for col, cities in enumerate(city_list):
        for row, city in enumerate(cities):
            b1 = Button(bottom_frame, text=city,
                        font=("Courier"), width=2, command=lambda city=city: weather_click(city, bottom_frame, bottom_frame2, bottom_frame3))
            b1.grid(column=col, row=row, padx=5, pady=15)
...
def draw_covid_btns(city_list, emo):
    ...
    for col, cities in enumerate(city_list):
        for row, city in enumerate(cities):
            b1 = Button(bottom_frame, text=city,
                        font=("Courier"), width=2, command=lambda city=city: covid_click(city, bottom_frame, bottom_frame2, bottom_frame3))
            b1.grid(column=col, row=row, padx=5, pady=15)
    re = Button(bottom_frame2, text=emo, width=30,
...
def btn_click(txt, bottom_frame, bottom_frame2):
    ...
    if txt == weather:
        ...
        target_num += txt
        # print(f"{target_num} Now!") - 타겟리스트 체크
...
top_frame = Frame(t, relief="sunken", bd=8)
...
entry.pack()
entry.insert(0, "0")
# covid = PhotoImage(file="covid.png")
